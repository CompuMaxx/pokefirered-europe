#if GAME_LANGUAGE == LANGUAGE_SPANISH
#define TEXT_DECORNAME_SMALL_DESK       _("MESA PEQUEÑA")
#define TEXT_DECORNAME_POKEMON_DESK     _("MESA POKéMON")
#define TEXT_DECORNAME_HEAVY_DESK       _("MESA PESADA")
#define TEXT_DECORNAME_RAGGED_DESK      _("MESA RÚSTICA")
#define TEXT_DECORNAME_COMFORT_DESK     _("MESA CÓMODA")
#define TEXT_DECORNAME_PRETTY_DESK      _("MESA BONITA")
#define TEXT_DECORNAME_BRICK_DESK       _("MESA LADRILLO")
#define TEXT_DECORNAME_CAMP_DESK        _("MESA DE CAMPO")
#define TEXT_DECORNAME_HARD_DESK        _("MESA FUERTE")
#define TEXT_DECORNAME_SMALL_CHAIR      _("SILLA PEQUEÑA")
#define TEXT_DECORNAME_POKEMON_CHAIR    _("SILLA POKéMON")
#define TEXT_DECORNAME_HEAVY_CHAIR      _("SILLA PESADA")
#define TEXT_DECORNAME_PRETTY_CHAIR     _("SILLA BONITA")
#define TEXT_DECORNAME_COMFORT_CHAIR    _("SILLA CÓMODA")
#define TEXT_DECORNAME_RAGGED_CHAIR     _("SILLA RÚSTICA")
#define TEXT_DECORNAME_BRICK_CHAIR      _("SILLA LADRILLO")
#define TEXT_DECORNAME_CAMP_CHAIR       _("SILLA DE CAMPO")
#define TEXT_DECORNAME_HARD_CHAIR       _("SILLA FUERTE")
#define TEXT_DECORNAME_RED_PLANT        _("PLANTA ROJA")
#define TEXT_DECORNAME_TROPICAL_PLANT   _("PL. TROPICAL")
#define TEXT_DECORNAME_PRETTY_FLOWERS   _("FLORES BONITAS")
#define TEXT_DECORNAME_COLORFUL_PLANT   _("PL. COLORIDA")
#define TEXT_DECORNAME_BIG_PLANT        _("PLANTA GRANDE")
#define TEXT_DECORNAME_GORGEOUS_PLANT   _("PLANTA ENORME")
#define TEXT_DECORNAME_RED_BRICK        _("LADRILLO ROJO")
#define TEXT_DECORNAME_YELLOW_BRICK     _("LADR. AMARILLO")
#define TEXT_DECORNAME_BLUE_BRICK       _("LADRILLO AZUL")
#define TEXT_DECORNAME_RED_BALLOON      _("GLOBO ROJO")
#define TEXT_DECORNAME_BLUE_BALLOON     _("GLOBO AZUL")
#define TEXT_DECORNAME_YELLOW_BALLOON   _("GLOBO AMARILLO")
#define TEXT_DECORNAME_RED_TENT         _("TIENDA ROJA")
#define TEXT_DECORNAME_BLUE_TENT        _("TIENDA AZUL")
#define TEXT_DECORNAME_SOLID_BOARD      _("TABLA")
#define TEXT_DECORNAME_SLIDE            _("TOBOGÁN")
#define TEXT_DECORNAME_FENCE_LENGTH     _("VALLA LARGA")
#define TEXT_DECORNAME_FENCE_WIDTH      _("VALLA ANCHA")
#define TEXT_DECORNAME_TIRE             _("NEUMÁTICO")
#define TEXT_DECORNAME_STAND            _("PODIO")
#define TEXT_DECORNAME_MUD_BALL         _("BOLA DE LODO")
#define TEXT_DECORNAME_BREAKABLE_DOOR   _("PUERTA RARA")
#define TEXT_DECORNAME_SAND_ORNAMENT    _("ADORNO ARENA")
#define TEXT_DECORNAME_SILVER_SHIELD    _("ESCUDO PLATA")
#define TEXT_DECORNAME_GOLD_SHIELD      _("ESCUDO DE ORO")
#define TEXT_DECORNAME_GLASS_ORNAMENT   _("ADORNO CRISTAL")
#define TEXT_DECORNAME_TV               _("TELEVISIÓN")
#define TEXT_DECORNAME_ROUND_TV         _("TELE REDONDA")
#define TEXT_DECORNAME_CUTE_TV          _("TELE BONITA")
#define TEXT_DECORNAME_GLITTER_MAT      _("TAPETE BRILLO")
#define TEXT_DECORNAME_JUMP_MAT         _("TAPETE MUELLE")
#define TEXT_DECORNAME_SPIN_MAT         _("TAPETE GIRO")
#define TEXT_DECORNAME_C_LOW_NOTE_MAT   _("TAP. DO MENOR")
#define TEXT_DECORNAME_D_NOTE_MAT       _("TAPETE NOTA RE")
#define TEXT_DECORNAME_E_NOTE_MAT       _("TAPETE NOTA MI")
#define TEXT_DECORNAME_F_NOTE_MAT       _("TAPETE NOTA FA")
#define TEXT_DECORNAME_G_NOTE_MAT       _("TAP. NOTA SOL")
#define TEXT_DECORNAME_A_NOTE_MAT       _("TAPETE NOTA LA")
#define TEXT_DECORNAME_B_NOTE_MAT       _("TAPETE NOTA SI")
#define TEXT_DECORNAME_C_HIGH_NOTE_MAT  _("TAP. DO MAYOR")
#define TEXT_DECORNAME_SURF_MAT         _("TAPETE SURF")
#define TEXT_DECORNAME_THUNDER_MAT      _("TAPETE TRUENO")
#define TEXT_DECORNAME_FIRE_BLAST_MAT   _("TAP. LLAMARADA")
#define TEXT_DECORNAME_POWDER_SNOW_MAT  _("T. NIEVE POLVO")
#define TEXT_DECORNAME_ATTRACT_MAT      _("TAP. ATRACCIÓN")
#define TEXT_DECORNAME_FISSURE_MAT      _("TAPETE FISURA")
#define TEXT_DECORNAME_SPIKES_MAT       _("TAPETE PÚAS")
#define TEXT_DECORNAME_BALL_POSTER      _("P. POKé BALL")
#define TEXT_DECORNAME_GREEN_POSTER     _("PÓSTER VERDE")
#define TEXT_DECORNAME_RED_POSTER       _("PÓSTER ROJO")
#define TEXT_DECORNAME_BLUE_POSTER      _("PÓSTER AZUL")
#define TEXT_DECORNAME_CUTE_POSTER      _("PÓSTER BONITO")
#define TEXT_DECORNAME_PIKA_POSTER      _("PÓSTER PIKA")
#define TEXT_DECORNAME_LONG_POSTER      _("PÓSTER GRANDE")
#define TEXT_DECORNAME_SEA_POSTER       _("PÓSTER MAR")
#define TEXT_DECORNAME_SKY_POSTER       _("PÓSTER CIELO")
#define TEXT_DECORNAME_KISS_POSTER      _("PÓSTER BESO")
#define TEXT_DECORNAME_PICHU_DOLL       _("MUÑECO PICHU")
#define TEXT_DECORNAME_PIKACHU_DOLL     _("MUÑECO PIKACHU")
#define TEXT_DECORNAME_MARILL_DOLL      _("MUÑECO MARILL")
#define TEXT_DECORNAME_TOGEPI_DOLL      _("MUÑECO TOGEPI")
#define TEXT_DECORNAME_CYNDAQUIL_DOLL   _("MUÑ. CYNDAQUIL")
#define TEXT_DECORNAME_CHIKORITA_DOLL   _("MUÑ. CHIKORITA")
#define TEXT_DECORNAME_TOTODILE_DOLL    _("MUÑ. TOTODILE")
#define TEXT_DECORNAME_JIGGLYPUFF_DOLL  _("MUÑ. JIGGLYPUFF")
#define TEXT_DECORNAME_MEOWTH_DOLL      _("MUÑECO MEOWTH")
#define TEXT_DECORNAME_CLEFAIRY_DOLL    _("MUÑECO CLEFAIRY")
#define TEXT_DECORNAME_DITTO_DOLL       _("MUÑECO DITTO")
#define TEXT_DECORNAME_SMOOCHUM_DOLL    _("MUÑECO SMOOCHUM")
#define TEXT_DECORNAME_TREECKO_DOLL     _("MUÑECO TREECKO")
#define TEXT_DECORNAME_TORCHIC_DOLL     _("MUÑECO TORCHIC")
#define TEXT_DECORNAME_MUDKIP_DOLL      _("MUÑECO MUDKIP")
#define TEXT_DECORNAME_DUSKULL_DOLL     _("MUÑECO DUSKULL")
#define TEXT_DECORNAME_WYNAUT_DOLL      _("MUÑECO WYNAUT")
#define TEXT_DECORNAME_BALTOY_DOLL      _("MUÑECO BALTOY")
#define TEXT_DECORNAME_KECLEON_DOLL     _("MUÑECO KECLEON")
#define TEXT_DECORNAME_AZURILL_DOLL     _("MUÑECO AZURILL")
#define TEXT_DECORNAME_SKITTY_DOLL      _("MUÑECO SKITTY")
#define TEXT_DECORNAME_SWABLU_DOLL      _("MUÑECO SWABLU")
#define TEXT_DECORNAME_GULPIN_DOLL      _("MUÑECO GULPIN")
#define TEXT_DECORNAME_LOTAD_DOLL       _("MUÑECO LOTAD")
#define TEXT_DECORNAME_SEEDOT_DOLL      _("MUÑECO SEEDOT")
#define TEXT_DECORNAME_PIKA_CUSHION     _("COJÍN PIKA")
#define TEXT_DECORNAME_ROUND_CUSHION    _("COJÍN REDONDO")
#define TEXT_DECORNAME_KISS_CUSHION     _("COJÍN BESO")
#define TEXT_DECORNAME_ZIGZAG_CUSHION   _("COJÍN ZIGZAGOON")
#define TEXT_DECORNAME_SPIN_CUSHION     _("COJÍN SPINDA")
#define TEXT_DECORNAME_DIAMOND_CUSHION  _("COJÍN DIAMANTE")
#define TEXT_DECORNAME_BALL_CUSHION     _("COJÍN BALL")
#define TEXT_DECORNAME_GRASS_CUSHION    _("COJÍN PLANTA")
#define TEXT_DECORNAME_FIRE_CUSHION     _("COJÍN FUEGO")
#define TEXT_DECORNAME_WATER_CUSHION    _("COJÍN AGUA")
#define TEXT_DECORNAME_SNORLAX_DOLL     _("MUÑECO SNORLAX")
#define TEXT_DECORNAME_RHYDON_DOLL      _("MUÑECO RHYDON")
#define TEXT_DECORNAME_LAPRAS_DOLL      _("MUÑECO LAPRAS")
#define TEXT_DECORNAME_VENUSAUR_DOLL    _("MUÑECO VENUSAUR")
#define TEXT_DECORNAME_CHARIZAR_DOLL    _("MUÑECO CHARIZAR")
#define TEXT_DECORNAME_BLASTOISE_DOLL   _("MUÑ. BLASTOISE")
#define TEXT_DECORNAME_WAILMER_DOLL     _("MUÑECO WAILMER")
#define TEXT_DECORNAME_REGIROCK_DOLL    _("MUÑECO REGIROCK")
#define TEXT_DECORNAME_REGICE_DOLL      _("MUÑECO REGICE")
#define TEXT_DECORNAME_REGISTEEL_DOLL   _("MUÑ. REGISTEEL")

#elif GAME_LANGUAGE == LANGUAGE_ITALIAN
#define TEXT_DECORNAME_SMALL_DESK       _("TAVOLO PICCOLO")
#define TEXT_DECORNAME_POKEMON_DESK     _("TAVOLO POKéMON")
#define TEXT_DECORNAME_HEAVY_DESK       _("TAVOLO PESANTE")
#define TEXT_DECORNAME_RAGGED_DESK      _("TAVOLO GREZZO")
#define TEXT_DECORNAME_COMFORT_DESK     _("TAVOLO COMODO")
#define TEXT_DECORNAME_PRETTY_DESK      _("BEL TAVOLO")
#define TEXT_DECORNAME_BRICK_DESK       _("TAVOLO MATTONI")
#define TEXT_DECORNAME_CAMP_DESK        _("TAVOLO DA CAMPO")
#define TEXT_DECORNAME_HARD_DESK        _("TAVOLO DURO")
#define TEXT_DECORNAME_SMALL_CHAIR      _("SEDIA PICCOLA")
#define TEXT_DECORNAME_POKEMON_CHAIR    _("SEDIA POKéMON")
#define TEXT_DECORNAME_HEAVY_CHAIR      _("SEDIA PESANTE")
#define TEXT_DECORNAME_PRETTY_CHAIR     _("BELLA SEDIA")
#define TEXT_DECORNAME_COMFORT_CHAIR    _("SEDIA COMODA")
#define TEXT_DECORNAME_RAGGED_CHAIR     _("SEDIA GREZZA")
#define TEXT_DECORNAME_BRICK_CHAIR      _("SEDIA MATTONI")
#define TEXT_DECORNAME_CAMP_CHAIR       _("SEDIA DA CAMPO")
#define TEXT_DECORNAME_HARD_CHAIR       _("SEDIA DURA")
#define TEXT_DECORNAME_RED_PLANT        _("PIANTA ROSSA")
#define TEXT_DECORNAME_TROPICAL_PLANT   _("TROPICPIANTA")
#define TEXT_DECORNAME_PRETTY_FLOWERS   _("BEI FIORI")
#define TEXT_DECORNAME_COLORFUL_PLANT   _("PIANTA COLORATA")
#define TEXT_DECORNAME_BIG_PLANT        _("PIANTA GRANDE")
#define TEXT_DECORNAME_GORGEOUS_PLANT   _("PIANTA CHIC")
#define TEXT_DECORNAME_RED_BRICK        _("MATTONE ROSSO")
#define TEXT_DECORNAME_YELLOW_BRICK     _("MATTONE GIALLO")
#define TEXT_DECORNAME_BLUE_BRICK       _("MATTONE BLU")
#define TEXT_DECORNAME_RED_BALLOON      _("PALLONE ROSSO")
#define TEXT_DECORNAME_BLUE_BALLOON     _("PALLONE BLU")
#define TEXT_DECORNAME_YELLOW_BALLOON   _("PALLONE GIALLO")
#define TEXT_DECORNAME_RED_TENT         _("TENDA ROSSA")
#define TEXT_DECORNAME_BLUE_TENT        _("TENDA BLU")
#define TEXT_DECORNAME_SOLID_BOARD      _("ASSE SOLIDA")
#define TEXT_DECORNAME_SLIDE            _("SCIVOLO")
#define TEXT_DECORNAME_FENCE_LENGTH     _("BARRIERA LUNGA")
#define TEXT_DECORNAME_FENCE_WIDTH      _("BARRIERA LARGA")
#define TEXT_DECORNAME_TIRE             _("CERCHIONE")
#define TEXT_DECORNAME_STAND            _("PALCHETTO")
#define TEXT_DECORNAME_MUD_BALL         _("PALLA FANGO")
#define TEXT_DECORNAME_BREAKABLE_DOOR   _("PORTA FRAGILE")
#define TEXT_DECORNAME_SAND_ORNAMENT    _("STATUA FRAGILE")
#define TEXT_DECORNAME_SILVER_SHIELD    _("SCUDO ARGENTEO")
#define TEXT_DECORNAME_GOLD_SHIELD      _("SCUDO AUREO")
#define TEXT_DECORNAME_GLASS_ORNAMENT   _("STATUA VITREA")
#define TEXT_DECORNAME_TV               _("TV")
#define TEXT_DECORNAME_ROUND_TV         _("TV ROTONDA")
#define TEXT_DECORNAME_CUTE_TV          _("TV CARINA")
#define TEXT_DECORNAME_GLITTER_MAT      _("TAPP. BRILLANTE")
#define TEXT_DECORNAME_JUMP_MAT         _("TAPPETO SALTO")
#define TEXT_DECORNAME_SPIN_MAT         _("TAPPETO GIRO")
#define TEXT_DECORNAME_C_LOW_NOTE_MAT   _("TAPP. DO BASSO")
#define TEXT_DECORNAME_D_NOTE_MAT       _("TAPPETO RE")
#define TEXT_DECORNAME_E_NOTE_MAT       _("TAPPETO MI")
#define TEXT_DECORNAME_F_NOTE_MAT       _("TAPPETO FA")
#define TEXT_DECORNAME_G_NOTE_MAT       _("TAPPETO SOL")
#define TEXT_DECORNAME_A_NOTE_MAT       _("TAPPETO LA")
#define TEXT_DECORNAME_B_NOTE_MAT       _("TAPPETO SI")
#define TEXT_DECORNAME_C_HIGH_NOTE_MAT  _("TAPPETO DO ALTO")
#define TEXT_DECORNAME_SURF_MAT         _("TAPPETO SURF")
#define TEXT_DECORNAME_THUNDER_MAT      _("TAPPETO TUONO")
#define TEXT_DECORNAME_FIRE_BLAST_MAT   _("TAPP.FUOCOBOMBA")
#define TEXT_DECORNAME_POWDER_SNOW_MAT  _("TAPP. POLNEVE")
#define TEXT_DECORNAME_ATTRACT_MAT      _("TAPP.ATTRAZIONE")
#define TEXT_DECORNAME_FISSURE_MAT      _("TAPP. ABISSO")
#define TEXT_DECORNAME_SPIKES_MAT       _("TAPP. PUNTE")
#define TEXT_DECORNAME_BALL_POSTER      _("POSTER BALL")
#define TEXT_DECORNAME_GREEN_POSTER     _("POSTER VERDE")
#define TEXT_DECORNAME_RED_POSTER       _("POSTER ROSSO")
#define TEXT_DECORNAME_BLUE_POSTER      _("POSTER BLU")
#define TEXT_DECORNAME_CUTE_POSTER      _("POSTER CARINO")
#define TEXT_DECORNAME_PIKA_POSTER      _("POSTER PIKA")
#define TEXT_DECORNAME_LONG_POSTER      _("POSTER LUNGO")
#define TEXT_DECORNAME_SEA_POSTER       _("POSTER MARE")
#define TEXT_DECORNAME_SKY_POSTER       _("POSTER CIELO")
#define TEXT_DECORNAME_KISS_POSTER      _("POSTER SMACK")
#define TEXT_DECORNAME_PICHU_DOLL       _("BAMBOLA PICHU")
#define TEXT_DECORNAME_PIKACHU_DOLL     _("BAMBOLA PIKACHU")
#define TEXT_DECORNAME_MARILL_DOLL      _("BAMBOLA MARILL")
#define TEXT_DECORNAME_TOGEPI_DOLL      _("BAMBOLA TOGEPI")
#define TEXT_DECORNAME_CYNDAQUIL_DOLL   _("BAMB. CYNDAQUIL")
#define TEXT_DECORNAME_CHIKORITA_DOLL   _("BAMB. CHIKORITA")
#define TEXT_DECORNAME_TOTODILE_DOLL    _("BAMB. TOTODILE")
#define TEXT_DECORNAME_JIGGLYPUFF_DOLL  _("BAMB.JIGGLYPUFF")
#define TEXT_DECORNAME_MEOWTH_DOLL      _("BAMB. MEOWTH")
#define TEXT_DECORNAME_CLEFAIRY_DOLL    _("BAMB. CLEFAIRY")
#define TEXT_DECORNAME_DITTO_DOLL       _("BAMBOLA DITTO")
#define TEXT_DECORNAME_SMOOCHUM_DOLL    _("BAMB. SMOOCHUM")
#define TEXT_DECORNAME_TREECKO_DOLL     _("BAMBOLA TREECKO")
#define TEXT_DECORNAME_TORCHIC_DOLL     _("BAMBOLA TORCHIC")
#define TEXT_DECORNAME_MUDKIP_DOLL      _("BAMBOLA MUDKIP")
#define TEXT_DECORNAME_DUSKULL_DOLL     _("BAMBOLA DUSKULL")
#define TEXT_DECORNAME_WYNAUT_DOLL      _("BAMBOLA WYNAUT")
#define TEXT_DECORNAME_BALTOY_DOLL      _("BAMBOLA BALTOY")
#define TEXT_DECORNAME_KECLEON_DOLL     _("BAMBOLA KECLEON")
#define TEXT_DECORNAME_AZURILL_DOLL     _("BAMBOLA AZURILL")
#define TEXT_DECORNAME_SKITTY_DOLL      _("BAMBOLA SKITTY")
#define TEXT_DECORNAME_SWABLU_DOLL      _("BAMBOLA SWABLU")
#define TEXT_DECORNAME_GULPIN_DOLL      _("BAMBOLA GULPIN")
#define TEXT_DECORNAME_LOTAD_DOLL       _("BAMBOLA LOTAD")
#define TEXT_DECORNAME_SEEDOT_DOLL      _("BAMBOLA SEEDOT")
#define TEXT_DECORNAME_PIKA_CUSHION     _("CUSCINO PIKA")
#define TEXT_DECORNAME_ROUND_CUSHION    _("CUSCINO ROTONDO")
#define TEXT_DECORNAME_KISS_CUSHION     _("CUSCINO SMACK")
#define TEXT_DECORNAME_ZIGZAG_CUSHION   _("CUSCINO ZIGZAG")
#define TEXT_DECORNAME_SPIN_CUSHION     _("CUSCINO GIRO")
#define TEXT_DECORNAME_DIAMOND_CUSHION  _("CUSC. DIAMANTE")
#define TEXT_DECORNAME_BALL_CUSHION     _("CUSCINO BALL")
#define TEXT_DECORNAME_GRASS_CUSHION    _("CUSCINO ERBA")
#define TEXT_DECORNAME_FIRE_CUSHION     _("CUSCINO FUOCO")
#define TEXT_DECORNAME_WATER_CUSHION    _("CUSCINO ACQUA")
#define TEXT_DECORNAME_SNORLAX_DOLL     _("BAMBOLA SNORLAX")
#define TEXT_DECORNAME_RHYDON_DOLL      _("BAMBOLA RHYDON")
#define TEXT_DECORNAME_LAPRAS_DOLL      _("BAMBOLA LAPRAS")
#define TEXT_DECORNAME_VENUSAUR_DOLL    _("BAMB. VENUSAUR")
#define TEXT_DECORNAME_CHARIZAR_DOLL    _("BAMB. CHARIZARD")
#define TEXT_DECORNAME_BLASTOISE_DOLL   _("BAMB. BLASTOISE")
#define TEXT_DECORNAME_WAILMER_DOLL     _("BAMB. WAILMER")
#define TEXT_DECORNAME_REGIROCK_DOLL    _("BAMB. REGIROCK")
#define TEXT_DECORNAME_REGICE_DOLL      _("BAMBOLA REGICE")
#define TEXT_DECORNAME_REGISTEEL_DOLL   _("BAMB. REGISTEEL")

#elif GAME_LANGUAGE == LANGUAGE_FRENCH
#define TEXT_DECORNAME_SMALL_DESK       _("PETIT BUREAU")
#define TEXT_DECORNAME_POKEMON_DESK     _("BUREAU POKéMON")
#define TEXT_DECORNAME_HEAVY_DESK       _("GRAND BUREAU")
#define TEXT_DECORNAME_RAGGED_DESK      _("BUREAU BRUT")
#define TEXT_DECORNAME_COMFORT_DESK     _("BUREAU COQUET")
#define TEXT_DECORNAME_PRETTY_DESK      _("JOLI BUREAU")
#define TEXT_DECORNAME_BRICK_DESK       _("BUREAU BRIQUES")
#define TEXT_DECORNAME_CAMP_DESK        _("BUREAU CAMPEUR")
#define TEXT_DECORNAME_HARD_DESK        _("BUREAU PIERRES")
#define TEXT_DECORNAME_SMALL_CHAIR      _("PETITE CHAISE")
#define TEXT_DECORNAME_POKEMON_CHAIR    _("CHAISE POKéMON")
#define TEXT_DECORNAME_HEAVY_CHAIR      _("GRANDE CHAISE")
#define TEXT_DECORNAME_PRETTY_CHAIR     _("JOLIE CHAISE")
#define TEXT_DECORNAME_COMFORT_CHAIR    _("CHAISE COQUETTE")
#define TEXT_DECORNAME_RAGGED_CHAIR     _("CHAISE BRUTE")
#define TEXT_DECORNAME_BRICK_CHAIR      _("CHAISE BRIQUES")
#define TEXT_DECORNAME_CAMP_CHAIR       _("CHAISE CAMPEUR")
#define TEXT_DECORNAME_HARD_CHAIR       _("CHAISE PIERRES")
#define TEXT_DECORNAME_RED_PLANT        _("PLANTE ROUGE")
#define TEXT_DECORNAME_TROPICAL_PLANT   _("PLANTE TROPIC")
#define TEXT_DECORNAME_PRETTY_FLOWERS   _("JOLIES FLEURS")
#define TEXT_DECORNAME_COLORFUL_PLANT   _("PLANTE COLOREE")
#define TEXT_DECORNAME_BIG_PLANT        _("GROSSE PLANTE")
#define TEXT_DECORNAME_GORGEOUS_PLANT   _("BELLE PLANTE")
#define TEXT_DECORNAME_RED_BRICK        _("BRIQUE ROUGE")
#define TEXT_DECORNAME_YELLOW_BRICK     _("BRIQUE JAUNE")
#define TEXT_DECORNAME_BLUE_BRICK       _("BRIQUE BLEUE")
#define TEXT_DECORNAME_RED_BALLOON      _("BALLON ROUGE")
#define TEXT_DECORNAME_BLUE_BALLOON     _("BALLON BLEU")
#define TEXT_DECORNAME_YELLOW_BALLOON   _("BALLON JAUNE")
#define TEXT_DECORNAME_RED_TENT         _("TENTE ROUGE")
#define TEXT_DECORNAME_BLUE_TENT        _("TENTE BLEUE")
#define TEXT_DECORNAME_SOLID_BOARD      _("PLANCHE SOLIDE")
#define TEXT_DECORNAME_SLIDE            _("TOBOGGAN")
#define TEXT_DECORNAME_FENCE_LENGTH     _("BARRIERE HAUTE")
#define TEXT_DECORNAME_FENCE_WIDTH      _("BARRIERE LARGE")
#define TEXT_DECORNAME_TIRE             _("PNEU")
#define TEXT_DECORNAME_STAND            _("ESTRADE")
#define TEXT_DECORNAME_MUD_BALL         _("BOULE DE BOUE")
#define TEXT_DECORNAME_BREAKABLE_DOOR   _("PORTE CASSABLE")
#define TEXT_DECORNAME_SAND_ORNAMENT    _("DECO DE SABLE")
#define TEXT_DECORNAME_SILVER_SHIELD    _("BOUCLIER ARGENT")
#define TEXT_DECORNAME_GOLD_SHIELD      _("BOUCLIER OR")
#define TEXT_DECORNAME_GLASS_ORNAMENT   _("ORNEMENT VERRE")
#define TEXT_DECORNAME_TV               _("TV")
#define TEXT_DECORNAME_ROUND_TV         _("TV RONDE")
#define TEXT_DECORNAME_CUTE_TV          _("JOLIE TV")
#define TEXT_DECORNAME_GLITTER_MAT      _("TAPIS BRILLANT")
#define TEXT_DECORNAME_JUMP_MAT         _("TAPIS SAUTEUR")
#define TEXT_DECORNAME_SPIN_MAT         _("TAPIS TOURNANT")
#define TEXT_DECORNAME_C_LOW_NOTE_MAT   _("TAPIS DO GRAVE")
#define TEXT_DECORNAME_D_NOTE_MAT       _("TAPIS RE")
#define TEXT_DECORNAME_E_NOTE_MAT       _("TAPIS MI")
#define TEXT_DECORNAME_F_NOTE_MAT       _("TAPIS FA")
#define TEXT_DECORNAME_G_NOTE_MAT       _("TAPIS SOL")
#define TEXT_DECORNAME_A_NOTE_MAT       _("TAPIS LA")
#define TEXT_DECORNAME_B_NOTE_MAT       _("TAPIS SI")
#define TEXT_DECORNAME_C_HIGH_NOTE_MAT  _("TAPIS DO AIGU")
#define TEXT_DECORNAME_SURF_MAT         _("TAPIS SURF")
#define TEXT_DECORNAME_THUNDER_MAT      _("TA.FATAL-FOUDRE")
#define TEXT_DECORNAME_FIRE_BLAST_MAT   _("TA.DEFLAGRATION")
#define TEXT_DECORNAME_POWDER_SNOW_MAT  _("TAPIS POUDREUSE")
#define TEXT_DECORNAME_ATTRACT_MAT      _("TAP. ATTRACTION")
#define TEXT_DECORNAME_FISSURE_MAT      _("TAPIS ABIME")
#define TEXT_DECORNAME_SPIKES_MAT       _("TAPIS PICOTS")
#define TEXT_DECORNAME_BALL_POSTER      _("POSTER BALL")
#define TEXT_DECORNAME_GREEN_POSTER     _("POSTER VERT")
#define TEXT_DECORNAME_RED_POSTER       _("POSTER ROUGE")
#define TEXT_DECORNAME_BLUE_POSTER      _("POSTER BLEU")
#define TEXT_DECORNAME_CUTE_POSTER      _("JOLI POSTER")
#define TEXT_DECORNAME_PIKA_POSTER      _("POSTER PIKA")
#define TEXT_DECORNAME_LONG_POSTER      _("GRAND POSTER")
#define TEXT_DECORNAME_SEA_POSTER       _("POSTER OCEAN")
#define TEXT_DECORNAME_SKY_POSTER       _("POSTER CIEL")
#define TEXT_DECORNAME_KISS_POSTER      _("POSTER BAISER")
#define TEXT_DECORNAME_PICHU_DOLL       _("POUPEE PICHU")
#define TEXT_DECORNAME_PIKACHU_DOLL     _("POUPEE PIKACHU")
#define TEXT_DECORNAME_MARILL_DOLL      _("POUPEE MARILL")
#define TEXT_DECORNAME_TOGEPI_DOLL      _("POUPEE TOGEPI")
#define TEXT_DECORNAME_CYNDAQUIL_DOLL   _("POUP.HERICENDRE")
#define TEXT_DECORNAME_CHIKORITA_DOLL   _("POUP. GERMIGNON")
#define TEXT_DECORNAME_TOTODILE_DOLL    _("POUPEE KAIMINUS")
#define TEXT_DECORNAME_JIGGLYPUFF_DOLL  _("POUP. RONDOUDOU")
#define TEXT_DECORNAME_MEOWTH_DOLL      _("POUPEE MIAOUSS")
#define TEXT_DECORNAME_CLEFAIRY_DOLL    _("POUPEE MELOFEE")
#define TEXT_DECORNAME_DITTO_DOLL       _("POUP. METAMORPH")
#define TEXT_DECORNAME_SMOOCHUM_DOLL    _("POUPEE LIPPOUTI")
#define TEXT_DECORNAME_TREECKO_DOLL     _("POUPEE ARCKO")
#define TEXT_DECORNAME_TORCHIC_DOLL     _("POUP.POUSSIFEU")
#define TEXT_DECORNAME_MUDKIP_DOLL      _("POUPEE GOBOU")
#define TEXT_DECORNAME_DUSKULL_DOLL     _("POUPEE SKELENOX")
#define TEXT_DECORNAME_WYNAUT_DOLL      _("POUPEE OKEOKE")
#define TEXT_DECORNAME_BALTOY_DOLL      _("POUPEE BALBUTO")
#define TEXT_DECORNAME_KECLEON_DOLL     _("POUPEE KECLEON")
#define TEXT_DECORNAME_AZURILL_DOLL     _("POUPEE AZURILL")
#define TEXT_DECORNAME_SKITTY_DOLL      _("POUPEE SKITTY")
#define TEXT_DECORNAME_SWABLU_DOLL      _("POUPEE TYLTON")
#define TEXT_DECORNAME_GULPIN_DOLL      _("POUPEE GLOUPTI")
#define TEXT_DECORNAME_LOTAD_DOLL       _("POUPEE NENUPIOT")
#define TEXT_DECORNAME_SEEDOT_DOLL      _("POUP.GRAINIPIOT")
#define TEXT_DECORNAME_PIKA_CUSHION     _("COUSSIN PIKACHU")
#define TEXT_DECORNAME_ROUND_CUSHION    _("COUSSIN ROND")
#define TEXT_DECORNAME_KISS_CUSHION     _("COUSSIN BAISER")
#define TEXT_DECORNAME_ZIGZAG_CUSHION   _("COUSSIN ZIGZAG")
#define TEXT_DECORNAME_SPIN_CUSHION     _("COUSSIN SPIRALE")
#define TEXT_DECORNAME_DIAMOND_CUSHION  _("COUSSIN DIAMANT")
#define TEXT_DECORNAME_BALL_CUSHION     _("COUSSIN BALL")
#define TEXT_DECORNAME_GRASS_CUSHION    _("COUSSIN PLANTE")
#define TEXT_DECORNAME_FIRE_CUSHION     _("COUSSIN FEU")
#define TEXT_DECORNAME_WATER_CUSHION    _("COUSSIN EAU")
#define TEXT_DECORNAME_SNORLAX_DOLL     _("POUPEE RONFLEX")
#define TEXT_DECORNAME_RHYDON_DOLL      _("POUP.RHINOFEROS")
#define TEXT_DECORNAME_LAPRAS_DOLL      _("POUPEE LOKHLASS")
#define TEXT_DECORNAME_VENUSAUR_DOLL    _("POUP.FLORIZARRE")
#define TEXT_DECORNAME_CHARIZAR_DOLL    _("POUP. DRACAUFEU")
#define TEXT_DECORNAME_BLASTOISE_DOLL   _("POUPEE TORTANK")
#define TEXT_DECORNAME_WAILMER_DOLL     _("POUPEE WAILMER")
#define TEXT_DECORNAME_REGIROCK_DOLL    _("POUPEE REGIROCK")
#define TEXT_DECORNAME_REGICE_DOLL      _("POUPEE REGICE")
#define TEXT_DECORNAME_REGISTEEL_DOLL   _("POUP. REGISTEEL")

#elif GAME_LANGUAGE == LANGUAGE_GERMAN
#define TEXT_DECORNAME_SMALL_DESK       _("KLEINER TISCH")
#define TEXT_DECORNAME_POKEMON_DESK     _("POKéMONTISCH")
#define TEXT_DECORNAME_HEAVY_DESK       _("SCHWERER TISCH")
#define TEXT_DECORNAME_RAGGED_DESK      _("INTARSIENTISCH")
#define TEXT_DECORNAME_COMFORT_DESK     _("NATURTISCH")
#define TEXT_DECORNAME_PRETTY_DESK      _("EDLER TISCH")
#define TEXT_DECORNAME_BRICK_DESK       _("BACKSTEINTISCH")
#define TEXT_DECORNAME_CAMP_DESK        _("CAMPINGTISCH")
#define TEXT_DECORNAME_HARD_DESK        _("MASSIVER TISCH")
#define TEXT_DECORNAME_SMALL_CHAIR      _("KLEINER STUHL")
#define TEXT_DECORNAME_POKEMON_CHAIR    _("POKéMON STUHL")
#define TEXT_DECORNAME_HEAVY_CHAIR      _("SCHWERER STUHL")
#define TEXT_DECORNAME_PRETTY_CHAIR     _("EDLER STUHL")
#define TEXT_DECORNAME_COMFORT_CHAIR    _("NATURSTUHL")
#define TEXT_DECORNAME_RAGGED_CHAIR     _("INTARSIENSTUHL")
#define TEXT_DECORNAME_BRICK_CHAIR      _("BACKSTEINSTUHL")
#define TEXT_DECORNAME_CAMP_CHAIR       _("CAMPINGSTUHL")
#define TEXT_DECORNAME_HARD_CHAIR       _("MASSIVER STUHL")
#define TEXT_DECORNAME_RED_PLANT        _("ROTE PFLANZE")
#define TEXT_DECORNAME_TROPICAL_PLANT   _("TROPENPFLANZE")
#define TEXT_DECORNAME_PRETTY_FLOWERS   _("HÜBSCHE BLUMEN")
#define TEXT_DECORNAME_COLORFUL_PLANT   _("BUNTE PFLANZE")
#define TEXT_DECORNAME_BIG_PLANT        _("GROSSE PFLANZE")
#define TEXT_DECORNAME_GORGEOUS_PLANT   _("ZIERPFLANZE")
#define TEXT_DECORNAME_RED_BRICK        _("ROTER STEIN")
#define TEXT_DECORNAME_YELLOW_BRICK     _("GELBER STEIN")
#define TEXT_DECORNAME_BLUE_BRICK       _("BLAUER STEIN")
#define TEXT_DECORNAME_RED_BALLOON      _("ROTER BALLON")
#define TEXT_DECORNAME_BLUE_BALLOON     _("BLAUER BALLON")
#define TEXT_DECORNAME_YELLOW_BALLOON   _("GELBER BALLON")
#define TEXT_DECORNAME_RED_TENT         _("ROTES ZELT")
#define TEXT_DECORNAME_BLUE_TENT        _("BLAUES ZELT")
#define TEXT_DECORNAME_SOLID_BOARD      _("SOLIDES BRETT")
#define TEXT_DECORNAME_SLIDE            _("RUTSCHE")
#define TEXT_DECORNAME_FENCE_LENGTH     _("ZAUN VERTIKAL")
#define TEXT_DECORNAME_FENCE_WIDTH      _("ZAUN HORIZONTAL")
#define TEXT_DECORNAME_TIRE             _("REIFEN")
#define TEXT_DECORNAME_STAND            _("PODEST")
#define TEXT_DECORNAME_MUD_BALL         _("LEHMBALL")
#define TEXT_DECORNAME_BREAKABLE_DOOR   _("BRECHBARE TÜR")
#define TEXT_DECORNAME_SAND_ORNAMENT    _("SANDORNAMENT")
#define TEXT_DECORNAME_SILVER_SHIELD    _("SILBERSCHILD")
#define TEXT_DECORNAME_GOLD_SHIELD      _("GOLDSCHILD")
#define TEXT_DECORNAME_GLASS_ORNAMENT   _("GLASORNAMENT")
#define TEXT_DECORNAME_TV               _("FERNSEHER")
#define TEXT_DECORNAME_ROUND_TV         _("RUNDER TV")
#define TEXT_DECORNAME_CUTE_TV          _("HÜBSCHER TV")
#define TEXT_DECORNAME_GLITTER_MAT      _("GLITZER-MATTE")
#define TEXT_DECORNAME_JUMP_MAT         _("SPRUNG-MATTE")
#define TEXT_DECORNAME_SPIN_MAT         _("DREH-MATTE")
#define TEXT_DECORNAME_C_LOW_NOTE_MAT   _("Tiefes C-MATTE")
#define TEXT_DECORNAME_D_NOTE_MAT       _("D-MATTE")
#define TEXT_DECORNAME_E_NOTE_MAT       _("E-MATTE")
#define TEXT_DECORNAME_F_NOTE_MAT       _("F-MATTE")
#define TEXT_DECORNAME_G_NOTE_MAT       _("G-MATTE")
#define TEXT_DECORNAME_A_NOTE_MAT       _("A-MATTE")
#define TEXT_DECORNAME_B_NOTE_MAT       _("H-MATTE")
#define TEXT_DECORNAME_C_HIGH_NOTE_MAT  _("Hohes C-MATTE")
#define TEXT_DECORNAME_SURF_MAT         _("SURFER-MATTE")
#define TEXT_DECORNAME_THUNDER_MAT      _("DONNER-MATTE")
#define TEXT_DECORNAME_FIRE_BLAST_MAT   _("FEUER-MATTE")
#define TEXT_DECORNAME_POWDER_SNOW_MAT  _("SCHNEE-MATTE")
#define TEXT_DECORNAME_ATTRACT_MAT      _("ANZIEHUNG-MAT.")
#define TEXT_DECORNAME_FISSURE_MAT      _("GEOFISSUR-MAT.")
#define TEXT_DECORNAME_SPIKES_MAT       _("STACHLER-MATTE")
#define TEXT_DECORNAME_BALL_POSTER      _("BALLPOSTER")
#define TEXT_DECORNAME_GREEN_POSTER     _("GRÜNES POSTER")
#define TEXT_DECORNAME_RED_POSTER       _("ROTES POSTER")
#define TEXT_DECORNAME_BLUE_POSTER      _("BLAUES POSTER")
#define TEXT_DECORNAME_CUTE_POSTER      _("HÜBSCHES POSTER")
#define TEXT_DECORNAME_PIKA_POSTER      _("PICHUKA-POSTER")
#define TEXT_DECORNAME_LONG_POSTER      _("SCHLANG-POSTER")
#define TEXT_DECORNAME_SEA_POSTER       _("MEER-POSTER")
#define TEXT_DECORNAME_SKY_POSTER       _("HIMMEL-POSTER")
#define TEXT_DECORNAME_KISS_POSTER      _("KUSS-POSTER")
#define TEXT_DECORNAME_PICHU_DOLL       _("PICHU-PUPPE")
#define TEXT_DECORNAME_PIKACHU_DOLL     _("PIKACHU-PUPPE")
#define TEXT_DECORNAME_MARILL_DOLL      _("MARILL-PUPPE")
#define TEXT_DECORNAME_TOGEPI_DOLL      _("TOGEPI-PUPPE")
#define TEXT_DECORNAME_CYNDAQUIL_DOLL   _("FEURIGEL-PUPPE")
#define TEXT_DECORNAME_CHIKORITA_DOLL   _("ENDIVIE-PUPPE")
#define TEXT_DECORNAME_TOTODILE_DOLL    _("KARNIMANIPUPPE")
#define TEXT_DECORNAME_JIGGLYPUFF_DOLL  _("PUMMELUFFPUPPE")
#define TEXT_DECORNAME_MEOWTH_DOLL      _("MAUZI-PUPPE")
#define TEXT_DECORNAME_CLEFAIRY_DOLL    _("PIEPI-PUPPE")
#define TEXT_DECORNAME_DITTO_DOLL       _("DITTO-PUPPE")
#define TEXT_DECORNAME_SMOOCHUM_DOLL    _("KUSSILLA-PUPPE")
#define TEXT_DECORNAME_TREECKO_DOLL     _("GECKARBOR-PUPP.")
#define TEXT_DECORNAME_TORCHIC_DOLL     _("FLEMMLI-PUPPE")
#define TEXT_DECORNAME_MUDKIP_DOLL      _("HYDROPI-PUPPE")
#define TEXT_DECORNAME_DUSKULL_DOLL     _("ZWIRRLICHTPUP.")
#define TEXT_DECORNAME_WYNAUT_DOLL      _("ISSO-PUPPE")
#define TEXT_DECORNAME_BALTOY_DOLL      _("PUPPANCE-PUPPE")
#define TEXT_DECORNAME_KECLEON_DOLL     _("KECLEON-PUPPE")
#define TEXT_DECORNAME_AZURILL_DOLL     _("AZURILL-PUPPE")
#define TEXT_DECORNAME_SKITTY_DOLL      _("ENECO-PUPPE")
#define TEXT_DECORNAME_SWABLU_DOLL      _("WABLU-PUPPE")
#define TEXT_DECORNAME_GULPIN_DOLL      _("SCHLUPPUCKPUP.")
#define TEXT_DECORNAME_LOTAD_DOLL       _("LOTURZEL-PUPPE")
#define TEXT_DECORNAME_SEEDOT_DOLL      _("SAMURZEL-PUPPE")
#define TEXT_DECORNAME_PIKA_CUSHION     _("PIKA-KISSEN")
#define TEXT_DECORNAME_ROUND_CUSHION    _("RUNDES KISSEN")
#define TEXT_DECORNAME_KISS_CUSHION     _("KUSS-KISSEN")
#define TEXT_DECORNAME_ZIGZAG_CUSHION   _("ZIGZACHS-KISS.")
#define TEXT_DECORNAME_SPIN_CUSHION     _("KREISEL-KISSEN")
#define TEXT_DECORNAME_DIAMOND_CUSHION  _("DIAMANT-KISSEN")
#define TEXT_DECORNAME_BALL_CUSHION     _("BALL-KISSEN")
#define TEXT_DECORNAME_GRASS_CUSHION    _("PFLANZEN-KISS.")
#define TEXT_DECORNAME_FIRE_CUSHION     _("FEUER-KISSEN")
#define TEXT_DECORNAME_WATER_CUSHION    _("WASSER-KISSEN")
#define TEXT_DECORNAME_SNORLAX_DOLL     _("RELAXO-PUPPE")
#define TEXT_DECORNAME_RHYDON_DOLL      _("RIZEROS-PUPPE")
#define TEXT_DECORNAME_LAPRAS_DOLL      _("LAPRAS-PUPPE")
#define TEXT_DECORNAME_VENUSAUR_DOLL    _("BISAFLOR-PUPPE")
#define TEXT_DECORNAME_CHARIZAR_DOLL    _("GLURAK-PUPPE")
#define TEXT_DECORNAME_BLASTOISE_DOLL   _("TURTOK-PUPPE")
#define TEXT_DECORNAME_WAILMER_DOLL     _("WAILMER-PUPPE")
#define TEXT_DECORNAME_REGIROCK_DOLL    _("REGIROCK-PUPPE")
#define TEXT_DECORNAME_REGICE_DOLL      _("REGICE-PUPPE")
#define TEXT_DECORNAME_REGISTEEL_DOLL   _("REGISTEEL-PUPP.")

#else //LANGUAGE_ENGLISH
#define TEXT_DECORNAME_SMALL_DESK       _("SMALL DESK")
#define TEXT_DECORNAME_POKEMON_DESK     _("POKéMON DESK")
#define TEXT_DECORNAME_HEAVY_DESK       _("HEAVY DESK")
#define TEXT_DECORNAME_RAGGED_DESK      _("RAGGED DESK")
#define TEXT_DECORNAME_COMFORT_DESK     _("COMFORT DESK")
#define TEXT_DECORNAME_PRETTY_DESK      _("PRETTY DESK")
#define TEXT_DECORNAME_BRICK_DESK       _("BRICK DESK")
#define TEXT_DECORNAME_CAMP_DESK        _("CAMP DESK")
#define TEXT_DECORNAME_HARD_DESK        _("HARD DESK")
#define TEXT_DECORNAME_SMALL_CHAIR      _("SMALL CHAIR")
#define TEXT_DECORNAME_POKEMON_CHAIR    _("POKéMON CHAIR")
#define TEXT_DECORNAME_HEAVY_CHAIR      _("HEAVY CHAIR")
#define TEXT_DECORNAME_PRETTY_CHAIR     _("PRETTY CHAIR")
#define TEXT_DECORNAME_COMFORT_CHAIR    _("COMFORT CHAIR")
#define TEXT_DECORNAME_RAGGED_CHAIR     _("RAGGED CHAIR")
#define TEXT_DECORNAME_BRICK_CHAIR      _("BRICK CHAIR")
#define TEXT_DECORNAME_CAMP_CHAIR       _("CAMP CHAIR")
#define TEXT_DECORNAME_HARD_CHAIR       _("HARD CHAIR")
#define TEXT_DECORNAME_RED_PLANT        _("RED PLANT")
#define TEXT_DECORNAME_TROPICAL_PLANT   _("TROPICAL PLANT")
#define TEXT_DECORNAME_PRETTY_FLOWERS   _("PRETTY FLOWERS")
#define TEXT_DECORNAME_COLORFUL_PLANT   _("COLORFUL PLANT")
#define TEXT_DECORNAME_BIG_PLANT        _("BIG PLANT")
#define TEXT_DECORNAME_GORGEOUS_PLANT   _("GORGEOUS PLANT")
#define TEXT_DECORNAME_RED_BRICK        _("RED BRICK")
#define TEXT_DECORNAME_YELLOW_BRICK     _("YELLOW BRICK")
#define TEXT_DECORNAME_BLUE_BRICK       _("BLUE BRICK")
#define TEXT_DECORNAME_RED_BALLOON      _("RED BALLOON")
#define TEXT_DECORNAME_BLUE_BALLOON     _("BLUE BALLOON")
#define TEXT_DECORNAME_YELLOW_BALLOON   _("YELLOW BALLOON")
#define TEXT_DECORNAME_RED_TENT         _("RED TENT")
#define TEXT_DECORNAME_BLUE_TENT        _("BLUE TENT")
#define TEXT_DECORNAME_SOLID_BOARD      _("SOLID BOARD")
#define TEXT_DECORNAME_SLIDE            _("SLIDE")
#define TEXT_DECORNAME_FENCE_LENGTH     _("FENCE LENGTH")
#define TEXT_DECORNAME_FENCE_WIDTH      _("FENCE WIDTH")
#define TEXT_DECORNAME_TIRE             _("TIRE")
#define TEXT_DECORNAME_STAND            _("STAND")
#define TEXT_DECORNAME_MUD_BALL         _("MUD BALL")
#define TEXT_DECORNAME_BREAKABLE_DOOR   _("BREAKABLE DOOR")
#define TEXT_DECORNAME_SAND_ORNAMENT    _("SAND ORNAMENT")
#define TEXT_DECORNAME_SILVER_SHIELD    _("SILVER SHIELD")
#define TEXT_DECORNAME_GOLD_SHIELD      _("GOLD SHIELD")
#define TEXT_DECORNAME_GLASS_ORNAMENT   _("GLASS ORNAMENT")
#define TEXT_DECORNAME_TV               _("TV")
#define TEXT_DECORNAME_ROUND_TV         _("ROUND TV")
#define TEXT_DECORNAME_CUTE_TV          _("CUTE TV")
#define TEXT_DECORNAME_GLITTER_MAT      _("GLITTER MAT")
#define TEXT_DECORNAME_JUMP_MAT         _("JUMP MAT")
#define TEXT_DECORNAME_SPIN_MAT         _("SPIN MAT")
#define TEXT_DECORNAME_C_LOW_NOTE_MAT   _("C Low NOTE MAT")
#define TEXT_DECORNAME_D_NOTE_MAT       _("D NOTE MAT")
#define TEXT_DECORNAME_E_NOTE_MAT       _("E NOTE MAT")
#define TEXT_DECORNAME_F_NOTE_MAT       _("F NOTE MAT")
#define TEXT_DECORNAME_G_NOTE_MAT       _("G NOTE MAT")
#define TEXT_DECORNAME_A_NOTE_MAT       _("A NOTE MAT")
#define TEXT_DECORNAME_B_NOTE_MAT       _("B NOTE MAT")
#define TEXT_DECORNAME_C_HIGH_NOTE_MAT  _("C High NOTE MAT")
#define TEXT_DECORNAME_SURF_MAT         _("SURF MAT")
#define TEXT_DECORNAME_THUNDER_MAT      _("THUNDER MAT")
#define TEXT_DECORNAME_FIRE_BLAST_MAT   _("FIRE BLAST MAT")
#define TEXT_DECORNAME_POWDER_SNOW_MAT  _("POWDER SNOW MAT")
#define TEXT_DECORNAME_ATTRACT_MAT      _("ATTRACT MAT")
#define TEXT_DECORNAME_FISSURE_MAT      _("FISSURE MAT")
#define TEXT_DECORNAME_SPIKES_MAT       _("SPIKES MAT")
#define TEXT_DECORNAME_BALL_POSTER      _("BALL POSTER")
#define TEXT_DECORNAME_GREEN_POSTER     _("GREEN POSTER")
#define TEXT_DECORNAME_RED_POSTER       _("RED POSTER")
#define TEXT_DECORNAME_BLUE_POSTER      _("BLUE POSTER")
#define TEXT_DECORNAME_CUTE_POSTER      _("CUTE POSTER")
#define TEXT_DECORNAME_PIKA_POSTER      _("PIKA POSTER")
#define TEXT_DECORNAME_LONG_POSTER      _("LONG POSTER")
#define TEXT_DECORNAME_SEA_POSTER       _("SEA POSTER")
#define TEXT_DECORNAME_SKY_POSTER       _("SKY POSTER")
#define TEXT_DECORNAME_KISS_POSTER      _("KISS POSTER")
#define TEXT_DECORNAME_PICHU_DOLL       _("PICHU DOLL")
#define TEXT_DECORNAME_PIKACHU_DOLL     _("PIKACHU DOLL")
#define TEXT_DECORNAME_MARILL_DOLL      _("MARILL DOLL")
#define TEXT_DECORNAME_TOGEPI_DOLL      _("TOGEPI DOLL")
#define TEXT_DECORNAME_CYNDAQUIL_DOLL   _("CYNDAQUIL DOLL")
#define TEXT_DECORNAME_CHIKORITA_DOLL   _("CHIKORITA DOLL")
#define TEXT_DECORNAME_TOTODILE_DOLL    _("TOTODILE DOLL")
#define TEXT_DECORNAME_JIGGLYPUFF_DOLL  _("JIGGLYPUFF DOLL")
#define TEXT_DECORNAME_MEOWTH_DOLL      _("MEOWTH DOLL")
#define TEXT_DECORNAME_CLEFAIRY_DOLL    _("CLEFAIRY DOLL")
#define TEXT_DECORNAME_DITTO_DOLL       _("DITTO DOLL")
#define TEXT_DECORNAME_SMOOCHUM_DOLL    _("SMOOCHUM DOLL")
#define TEXT_DECORNAME_TREECKO_DOLL     _("TREECKO DOLL")
#define TEXT_DECORNAME_TORCHIC_DOLL     _("TORCHIC DOLL")
#define TEXT_DECORNAME_MUDKIP_DOLL      _("MUDKIP DOLL")
#define TEXT_DECORNAME_DUSKULL_DOLL     _("DUSKULL DOLL")
#define TEXT_DECORNAME_WYNAUT_DOLL      _("WYNAUT DOLL")
#define TEXT_DECORNAME_BALTOY_DOLL      _("BALTOY DOLL")
#define TEXT_DECORNAME_KECLEON_DOLL     _("KECLEON DOLL")
#define TEXT_DECORNAME_AZURILL_DOLL     _("AZURILL DOLL")
#define TEXT_DECORNAME_SKITTY_DOLL      _("SKITTY DOLL")
#define TEXT_DECORNAME_SWABLU_DOLL      _("SWABLU DOLL")
#define TEXT_DECORNAME_GULPIN_DOLL      _("GULPIN DOLL")
#define TEXT_DECORNAME_LOTAD_DOLL       _("LOTAD DOLL")
#define TEXT_DECORNAME_SEEDOT_DOLL      _("SEEDOT DOLL")
#define TEXT_DECORNAME_PIKA_CUSHION     _("PIKA CUSHION")
#define TEXT_DECORNAME_ROUND_CUSHION    _("ROUND CUSHION")
#define TEXT_DECORNAME_KISS_CUSHION     _("KISS CUSHION")
#define TEXT_DECORNAME_ZIGZAG_CUSHION   _("ZIGZAG CUSHION")
#define TEXT_DECORNAME_SPIN_CUSHION     _("SPIN CUSHION")
#define TEXT_DECORNAME_DIAMOND_CUSHION  _("DIAMOND CUSHION")
#define TEXT_DECORNAME_BALL_CUSHION     _("BALL CUSHION")
#define TEXT_DECORNAME_GRASS_CUSHION    _("GRASS CUSHION")
#define TEXT_DECORNAME_FIRE_CUSHION     _("FIRE CUSHION")
#define TEXT_DECORNAME_WATER_CUSHION    _("WATER CUSHION")
#define TEXT_DECORNAME_SNORLAX_DOLL     _("SNORLAX DOLL")
#define TEXT_DECORNAME_RHYDON_DOLL      _("RHYDON DOLL")
#define TEXT_DECORNAME_LAPRAS_DOLL      _("LAPRAS DOLL")
#define TEXT_DECORNAME_VENUSAUR_DOLL    _("VENUSAUR DOLL")
#define TEXT_DECORNAME_CHARIZAR_DOLL    _("CHARIZARD DOLL")
#define TEXT_DECORNAME_BLASTOISE_DOLL   _("BLASTOISE DOLL")
#define TEXT_DECORNAME_WAILMER_DOLL     _("WAILMER DOLL")
#define TEXT_DECORNAME_REGIROCK_DOLL    _("REGIROCK DOLL")
#define TEXT_DECORNAME_REGICE_DOLL      _("REGICE DOLL")
#define TEXT_DECORNAME_REGISTEEL_DOLL   _("REGISTEEL DOLL")
#endif

const struct Decoration gDecorations[] = {
    {
        DECOR_NONE,
        TEXT_DECORNAME_SMALL_DESK,
        DECORPERM_SOLID_FLOOR,
        DECORSHAPE_1x1,
        DECORCAT_DESK,
        0,
        DecorDesc_SMALL_DESK,
        DecorGfx_SMALL_DESK
    }, {
        DECOR_SMALL_DESK,
        TEXT_DECORNAME_SMALL_DESK,
        DECORPERM_SOLID_FLOOR,
        DECORSHAPE_1x1,
        DECORCAT_DESK,
        3000,
        DecorDesc_SMALL_DESK,
        DecorGfx_SMALL_DESK
    }, {
        DECOR_POKEMON_DESK,
        TEXT_DECORNAME_POKEMON_DESK,
        DECORPERM_SOLID_FLOOR,
        DECORSHAPE_1x1,
        DECORCAT_DESK,
        3000,
        DecorDesc_POKEMON_DESK,
        DecorGfx_POKEMON_DESK
    }, {
        DECOR_HEAVY_DESK,
        TEXT_DECORNAME_HEAVY_DESK,
        DECORPERM_SOLID_FLOOR,
        DECORSHAPE_3x2,
        DECORCAT_DESK,
        6000,
        DecorDesc_HEAVY_DESK,
        DecorGfx_HEAVY_DESK
    }, {
        DECOR_RAGGED_DESK,
        TEXT_DECORNAME_RAGGED_DESK,
        DECORPERM_SOLID_FLOOR,
        DECORSHAPE_3x2,
        DECORCAT_DESK,
        6000,
        DecorDesc_RAGGED_DESK,
        DecorGfx_RAGGED_DESK
    }, {
        DECOR_COMFORT_DESK,
        TEXT_DECORNAME_COMFORT_DESK,
        DECORPERM_SOLID_FLOOR,
        DECORSHAPE_3x2,
        DECORCAT_DESK,
        6000,
        DecorDesc_COMFORT_DESK,
        DecorGfx_COMFORT_DESK
    }, {
        DECOR_PRETTY_DESK,
        TEXT_DECORNAME_PRETTY_DESK,
        DECORPERM_SOLID_FLOOR,
        DECORSHAPE_3x3,
        DECORCAT_DESK,
        9000,
        DecorDesc_PRETTY_DESK,
        DecorGfx_PRETTY_DESK
    }, {
        DECOR_BRICK_DESK,
        TEXT_DECORNAME_BRICK_DESK,
        DECORPERM_SOLID_FLOOR,
        DECORSHAPE_3x3,
        DECORCAT_DESK,
        9000,
        DecorDesc_BRICK_DESK,
        DecorGfx_BRICK_DESK
    }, {
        DECOR_CAMP_DESK,
        TEXT_DECORNAME_CAMP_DESK,
        DECORPERM_SOLID_FLOOR,
        DECORSHAPE_3x3,
        DECORCAT_DESK,
        9000,
        DecorDesc_CAMP_DESK,
        DecorGfx_CAMP_DESK
    }, {
        DECOR_HARD_DESK,
        TEXT_DECORNAME_HARD_DESK,
        DECORPERM_SOLID_FLOOR,
        DECORSHAPE_3x3,
        DECORCAT_DESK,
        9000,
        DecorDesc_HARD_DESK,
        DecorGfx_HARD_DESK
    }, {
        DECOR_SMALL_CHAIR,
        TEXT_DECORNAME_SMALL_CHAIR,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_1x1,
        DECORCAT_CHAIR,
        2000,
        DecorDesc_SMALL_CHAIR,
        DecorGfx_SMALL_CHAIR
    }, {
        DECOR_POKEMON_CHAIR,
        TEXT_DECORNAME_POKEMON_CHAIR,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_1x1,
        DECORCAT_CHAIR,
        2000,
        DecorDesc_POKEMON_CHAIR,
        DecorGfx_POKEMON_CHAIR
    }, {
        DECOR_HEAVY_CHAIR,
        TEXT_DECORNAME_HEAVY_CHAIR,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_1x1,
        DECORCAT_CHAIR,
        2000,
        DecorDesc_HEAVY_CHAIR,
        DecorGfx_HEAVY_CHAIR
    }, {
        DECOR_PRETTY_CHAIR,
        TEXT_DECORNAME_PRETTY_CHAIR,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_1x1,
        DECORCAT_CHAIR,
        2000,
        DecorDesc_PRETTY_CHAIR,
        DecorGfx_PRETTY_CHAIR
    }, {
        DECOR_COMFORT_CHAIR,
        TEXT_DECORNAME_COMFORT_CHAIR,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_1x1,
        DECORCAT_CHAIR,
        2000,
        DecorDesc_COMFORT_CHAIR,
        DecorGfx_COMFORT_CHAIR
    }, {
        DECOR_RAGGED_CHAIR,
        TEXT_DECORNAME_RAGGED_CHAIR,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_1x1,
        DECORCAT_CHAIR,
        2000,
        DecorDesc_RAGGED_CHAIR,
        DecorGfx_RAGGED_CHAIR
    }, {
        DECOR_BRICK_CHAIR,
        TEXT_DECORNAME_BRICK_CHAIR,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_1x1,
        DECORCAT_CHAIR,
        2000,
        DecorDesc_BRICK_CHAIR,
        DecorGfx_BRICK_CHAIR
    }, {
        DECOR_CAMP_CHAIR,
        TEXT_DECORNAME_CAMP_CHAIR,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_1x1,
        DECORCAT_CHAIR,
        2000,
        DecorDesc_CAMP_CHAIR,
        DecorGfx_CAMP_CHAIR
    }, {
        DECOR_HARD_CHAIR,
        TEXT_DECORNAME_HARD_CHAIR,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_1x1,
        DECORCAT_CHAIR,
        2000,
        DecorDesc_HARD_CHAIR,
        DecorGfx_HARD_CHAIR
    }, {
        DECOR_RED_PLANT,
        TEXT_DECORNAME_RED_PLANT,
        DECORPERM_BEHIND_FLOOR,
        DECORSHAPE_1x2,
        DECORCAT_PLANT,
        3000,
        DecorDesc_RED_PLANT,
        DecorGfx_RED_PLANT
    }, {
        DECOR_TROPICAL_PLANT,
        TEXT_DECORNAME_TROPICAL_PLANT,
        DECORPERM_BEHIND_FLOOR,
        DECORSHAPE_1x2,
        DECORCAT_PLANT,
        3000,
        DecorDesc_TROPICAL_PLANT,
        DecorGfx_TROPICAL_PLANT
    }, {
        DECOR_PRETTY_FLOWERS,
        TEXT_DECORNAME_PRETTY_FLOWERS,
        DECORPERM_BEHIND_FLOOR,
        DECORSHAPE_1x2,
        DECORCAT_PLANT,
        3000,
        DecorDesc_PRETTY_FLOWERS,
        DecorGfx_PRETTY_FLOWERS
    }, {
        DECOR_COLORFUL_PLANT,
        TEXT_DECORNAME_COLORFUL_PLANT,
        DECORPERM_BEHIND_FLOOR,
        DECORSHAPE_2x2,
        DECORCAT_PLANT,
        5000,
        DecorDesc_COLORFUL_PLANT,
        DecorGfx_COLORFUL_PLANT
    }, {
        DECOR_BIG_PLANT,
        TEXT_DECORNAME_BIG_PLANT,
        DECORPERM_BEHIND_FLOOR,
        DECORSHAPE_2x2,
        DECORCAT_PLANT,
        5000,
        DecorDesc_BIG_PLANT,
        DecorGfx_BIG_PLANT
    }, {
        DECOR_GORGEOUS_PLANT,
        TEXT_DECORNAME_GORGEOUS_PLANT,
        DECORPERM_BEHIND_FLOOR,
        DECORSHAPE_2x2,
        DECORCAT_PLANT,
        5000,
        DecorDesc_GORGEOUS_PLANT,
        DecorGfx_GORGEOUS_PLANT
    }, {
        DECOR_RED_BRICK,
        TEXT_DECORNAME_RED_BRICK,
        DECORPERM_SOLID_FLOOR,
        DECORSHAPE_1x2,
        DECORCAT_ORNAMENT,
        500,
        DecorDesc_RED_BRICK,
        DecorGfx_RED_BRICK
    }, {
        DECOR_YELLOW_BRICK,
        TEXT_DECORNAME_YELLOW_BRICK,
        DECORPERM_SOLID_FLOOR,
        DECORSHAPE_1x2,
        DECORCAT_ORNAMENT,
        500,
        DecorDesc_YELLOW_BRICK,
        DecorGfx_YELLOW_BRICK
    }, {
        DECOR_BLUE_BRICK,
        TEXT_DECORNAME_BLUE_BRICK,
        DECORPERM_SOLID_FLOOR,
        DECORSHAPE_1x2,
        DECORCAT_ORNAMENT,
        500,
        DecorDesc_BLUE_BRICK,
        DecorGfx_BLUE_BRICK
    }, {
        DECOR_RED_BALLOON,
        TEXT_DECORNAME_RED_BALLOON,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_1x1,
        DECORCAT_ORNAMENT,
        500,
        DecorDesc_RED_BALLOON,
        DecorGfx_RED_BALLOON
    }, {
        DECOR_BLUE_BALLOON,
        TEXT_DECORNAME_BLUE_BALLOON,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_1x1,
        DECORCAT_ORNAMENT,
        500,
        DecorDesc_BLUE_BALLOON,
        DecorGfx_BLUE_BALLOON
    }, {
        DECOR_YELLOW_BALLOON,
        TEXT_DECORNAME_YELLOW_BALLOON,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_1x1,
        DECORCAT_ORNAMENT,
        500,
        DecorDesc_YELLOW_BALLOON,
        DecorGfx_YELLOW_BALLOON
    }, {
        DECOR_RED_TENT,
        TEXT_DECORNAME_RED_TENT,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_3x3,
        DECORCAT_ORNAMENT,
        10000,
        DecorDesc_RED_TENT,
        DecorGfx_RED_TENT
    }, {
        DECOR_BLUE_TENT,
        TEXT_DECORNAME_BLUE_TENT,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_3x3,
        DECORCAT_ORNAMENT,
        10000,
        DecorDesc_BLUE_TENT,
        DecorGfx_BLUE_TENT
    }, {
        DECOR_SOLID_BOARD,
        TEXT_DECORNAME_SOLID_BOARD,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_1x2,
        DECORCAT_ORNAMENT,
        3000,
        DecorDesc_SOLID_BOARD,
        DecorGfx_SOLID_BOARD
    }, {
        DECOR_SLIDE,
        TEXT_DECORNAME_SLIDE,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_2x4,
        DECORCAT_ORNAMENT,
        8000,
        DecorDesc_SLIDE,
        DecorGfx_SLIDE
    }, {
        DECOR_FENCE_LENGTH,
        TEXT_DECORNAME_FENCE_LENGTH,
        DECORPERM_SOLID_FLOOR,
        DECORSHAPE_1x1,
        DECORCAT_ORNAMENT,
        500,
        DecorDesc_FENCE_LENGTH,
        DecorGfx_FENCE_LENGTH
    }, {
        DECOR_FENCE_WIDTH,
        TEXT_DECORNAME_FENCE_WIDTH,
        DECORPERM_SOLID_FLOOR,
        DECORSHAPE_1x1,
        DECORCAT_ORNAMENT,
        500,
        DecorDesc_FENCE_WIDTH,
        DecorGfx_FENCE_WIDTH
    }, {
        DECOR_TIRE,
        TEXT_DECORNAME_TIRE,
        DECORPERM_SOLID_FLOOR,
        DECORSHAPE_2x2,
        DECORCAT_ORNAMENT,
        800,
        DecorDesc_TIRE,
        DecorGfx_TIRE
    }, {
        DECOR_STAND,
        TEXT_DECORNAME_STAND,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_4x2,
        DECORCAT_ORNAMENT,
        7000,
        DecorDesc_STAND,
        DecorGfx_STAND
    }, {
        DECOR_MUD_BALL,
        TEXT_DECORNAME_MUD_BALL,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_1x1,
        DECORCAT_ORNAMENT,
        200,
        DecorDesc_MUD_BALL,
        DecorGfx_MUD_BALL
    }, {
        DECOR_BREAKABLE_DOOR,
        TEXT_DECORNAME_BREAKABLE_DOOR,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_1x2,
        DECORCAT_ORNAMENT,
        3000,
        DecorDesc_BREAKABLE_DOOR,
        DecorGfx_BREAKABLE_DOOR
    }, {
        DECOR_SAND_ORNAMENT,
        TEXT_DECORNAME_SAND_ORNAMENT,
        DECORPERM_BEHIND_FLOOR,
        DECORSHAPE_1x2,
        DECORCAT_ORNAMENT,
        3000,
        DecorDesc_SAND_ORNAMENT,
        DecorGfx_SAND_ORNAMENT
    }, {
        DECOR_SILVER_SHIELD,
        TEXT_DECORNAME_SILVER_SHIELD,
        DECORPERM_BEHIND_FLOOR,
        DECORSHAPE_1x2,
        DECORCAT_ORNAMENT,
        0,
        DecorDesc_SILVER_SHIELD,
        DecorGfx_SILVER_SHIELD
    }, {
        DECOR_GOLD_SHIELD,
        TEXT_DECORNAME_GOLD_SHIELD,
        DECORPERM_BEHIND_FLOOR,
        DECORSHAPE_1x2,
        DECORCAT_ORNAMENT,
        0,
        DecorDesc_GOLD_SHIELD,
        DecorGfx_GOLD_SHIELD
    }, {
        DECOR_GLASS_ORNAMENT,
        TEXT_DECORNAME_GLASS_ORNAMENT,
        DECORPERM_BEHIND_FLOOR,
        DECORSHAPE_1x2,
        DECORCAT_ORNAMENT,
        0,
        DecorDesc_GLASS_ORNAMENT,
        DecorGfx_GLASS_ORNAMENT
    }, {
        DECOR_TV,
        TEXT_DECORNAME_TV,
        DECORPERM_SOLID_FLOOR,
        DECORSHAPE_1x1,
        DECORCAT_ORNAMENT,
        3000,
        DecorDesc_TV,
        DecorGfx_TV
    }, {
        DECOR_ROUND_TV,
        TEXT_DECORNAME_ROUND_TV,
        DECORPERM_SOLID_FLOOR,
        DECORSHAPE_1x1,
        DECORCAT_ORNAMENT,
        4000,
        DecorDesc_ROUND_TV,
        DecorGfx_ROUND_TV
    }, {
        DECOR_CUTE_TV,
        TEXT_DECORNAME_CUTE_TV,
        DECORPERM_SOLID_FLOOR,
        DECORSHAPE_1x1,
        DECORCAT_ORNAMENT,
        4000,
        DecorDesc_CUTE_TV,
        DecorGfx_CUTE_TV
    }, {
        DECOR_GLITTER_MAT,
        TEXT_DECORNAME_GLITTER_MAT,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_1x1,
        DECORCAT_MAT,
        2000,
        DecorDesc_GLITTER_MAT,
        DecorGfx_GLITTER_MAT
    }, {
        DECOR_JUMP_MAT,
        TEXT_DECORNAME_JUMP_MAT,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_1x1,
        DECORCAT_MAT,
        2000,
        DecorDesc_JUMP_MAT,
        DecorGfx_JUMP_MAT
    }, {
        DECOR_SPIN_MAT,
        TEXT_DECORNAME_SPIN_MAT,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_1x1,
        DECORCAT_MAT,
        2000,
        DecorDesc_SPIN_MAT,
        DecorGfx_SPIN_MAT
    }, {
        DECOR_C_LOW_NOTE_MAT,
        TEXT_DECORNAME_C_LOW_NOTE_MAT,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_1x1,
        DECORCAT_MAT,
        500,
        DecorDesc_C_LOW_NOTE_MAT,
        DecorGfx_C_LOW_NOTE_MAT
    }, {
        DECOR_D_NOTE_MAT,
        TEXT_DECORNAME_D_NOTE_MAT,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_1x1,
        DECORCAT_MAT,
        500,
        DecorDesc_D_NOTE_MAT,
        DecorGfx_D_NOTE_MAT
    }, {
        DECOR_E_NOTE_MAT,
        TEXT_DECORNAME_E_NOTE_MAT,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_1x1,
        DECORCAT_MAT,
        500,
        DecorDesc_E_NOTE_MAT,
        DecorGfx_E_NOTE_MAT
    }, {
        DECOR_F_NOTE_MAT,
        TEXT_DECORNAME_F_NOTE_MAT,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_1x1,
        DECORCAT_MAT,
        500,
        DecorDesc_F_NOTE_MAT,
        DecorGfx_F_NOTE_MAT
    }, {
        DECOR_G_NOTE_MAT,
        TEXT_DECORNAME_G_NOTE_MAT,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_1x1,
        DECORCAT_MAT,
        500,
        DecorDesc_G_NOTE_MAT,
        DecorGfx_G_NOTE_MAT
    }, {
        DECOR_A_NOTE_MAT,
        TEXT_DECORNAME_A_NOTE_MAT,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_1x1,
        DECORCAT_MAT,
        500,
        DecorDesc_A_NOTE_MAT,
        DecorGfx_A_NOTE_MAT
    }, {
        DECOR_B_NOTE_MAT,
        TEXT_DECORNAME_B_NOTE_MAT,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_1x1,
        DECORCAT_MAT,
        500,
        DecorDesc_B_NOTE_MAT,
        DecorGfx_B_NOTE_MAT
    }, {
        DECOR_C_HIGH_NOTE_MAT,
        TEXT_DECORNAME_C_HIGH_NOTE_MAT,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_1x1,
        DECORCAT_MAT,
        500,
        DecorDesc_C_HIGH_NOTE_MAT,
        DecorGfx_C_HIGH_NOTE_MAT
    }, {
        DECOR_SURF_MAT,
        TEXT_DECORNAME_SURF_MAT,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_3x3,
        DECORCAT_MAT,
        4000,
        DecorDesc_SURF_MAT,
        DecorGfx_SURF_MAT
    }, {
        DECOR_THUNDER_MAT,
        TEXT_DECORNAME_THUNDER_MAT,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_3x3,
        DECORCAT_MAT,
        4000,
        DecorDesc_THUNDER_MAT,
        DecorGfx_THUNDER_MAT
    }, {
        DECOR_FIRE_BLAST_MAT,
        TEXT_DECORNAME_FIRE_BLAST_MAT,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_3x3,
        DECORCAT_MAT,
        4000,
        DecorDesc_FIRE_BLAST_MAT,
        DecorGfx_FIRE_BLAST_MAT
    }, {
        DECOR_POWDER_SNOW_MAT,
        TEXT_DECORNAME_POWDER_SNOW_MAT,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_3x3,
        DECORCAT_MAT,
        4000,
        DecorDesc_POWDER_SNOW_MAT,
        DecorGfx_POWDER_SNOW_MAT
    }, {
        DECOR_ATTRACT_MAT,
        TEXT_DECORNAME_ATTRACT_MAT,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_3x3,
        DECORCAT_MAT,
        4000,
        DecorDesc_ATTRACT_MAT,
        DecorGfx_ATTRACT_MAT
    }, {
        DECOR_FISSURE_MAT,
        TEXT_DECORNAME_FISSURE_MAT,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_3x3,
        DECORCAT_MAT,
        4000,
        DecorDesc_FISSURE_MAT,
        DecorGfx_FISSURE_MAT
    }, {
        DECOR_SPIKES_MAT,
        TEXT_DECORNAME_SPIKES_MAT,
        DECORPERM_PASS_FLOOR,
        DECORSHAPE_3x3,
        DECORCAT_MAT,
        4000,
        DecorDesc_SPIKES_MAT,
        DecorGfx_SPIKES_MAT
    }, {
        DECOR_BALL_POSTER,
        TEXT_DECORNAME_BALL_POSTER,
        DECORPERM_NA_WALL,
        DECORSHAPE_1x1,
        DECORCAT_POSTER,
        1000,
        DecorDesc_BALL_POSTER,
        DecorGfx_BALL_POSTER
    }, {
        DECOR_GREEN_POSTER,
        TEXT_DECORNAME_GREEN_POSTER,
        DECORPERM_NA_WALL,
        DECORSHAPE_1x1,
        DECORCAT_POSTER,
        1000,
        DecorDesc_GREEN_POSTER,
        DecorGfx_GREEN_POSTER
    }, {
        DECOR_RED_POSTER,
        TEXT_DECORNAME_RED_POSTER,
        DECORPERM_NA_WALL,
        DECORSHAPE_1x1,
        DECORCAT_POSTER,
        1000,
        DecorDesc_RED_POSTER,
        DecorGfx_RED_POSTER
    }, {
        DECOR_BLUE_POSTER,
        TEXT_DECORNAME_BLUE_POSTER,
        DECORPERM_NA_WALL,
        DECORSHAPE_1x1,
        DECORCAT_POSTER,
        1000,
        DecorDesc_BLUE_POSTER,
        DecorGfx_BLUE_POSTER
    }, {
        DECOR_CUTE_POSTER,
        TEXT_DECORNAME_CUTE_POSTER,
        DECORPERM_NA_WALL,
        DECORSHAPE_1x1,
        DECORCAT_POSTER,
        1000,
        DecorDesc_CUTE_POSTER,
        DecorGfx_CUTE_POSTER
    }, {
        DECOR_PIKA_POSTER,
        TEXT_DECORNAME_PIKA_POSTER,
        DECORPERM_NA_WALL,
        DECORSHAPE_2x1,
        DECORCAT_POSTER,
        1500,
        DecorDesc_PIKA_POSTER,
        DecorGfx_PIKA_POSTER
    }, {
        DECOR_LONG_POSTER,
        TEXT_DECORNAME_LONG_POSTER,
        DECORPERM_NA_WALL,
        DECORSHAPE_2x1,
        DECORCAT_POSTER,
        1500,
        DecorDesc_LONG_POSTER,
        DecorGfx_LONG_POSTER
    }, {
        DECOR_SEA_POSTER,
        TEXT_DECORNAME_SEA_POSTER,
        DECORPERM_NA_WALL,
        DECORSHAPE_2x1,
        DECORCAT_POSTER,
        1500,
        DecorDesc_SEA_POSTER,
        DecorGfx_SEA_POSTER
    }, {
        DECOR_SKY_POSTER,
        TEXT_DECORNAME_SKY_POSTER,
        DECORPERM_NA_WALL,
        DECORSHAPE_2x1,
        DECORCAT_POSTER,
        1500,
        DecorDesc_SKY_POSTER,
        DecorGfx_SKY_POSTER
    }, {
        DECOR_KISS_POSTER,
        TEXT_DECORNAME_KISS_POSTER,
        DECORPERM_NA_WALL,
        DECORSHAPE_2x1,
        DECORCAT_POSTER,
        1500,
        DecorDesc_KISS_POSTER,
        DecorGfx_KISS_POSTER
    }, {
        DECOR_PICHU_DOLL,
        TEXT_DECORNAME_PICHU_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_DOLL,
        3000,
        DecorDesc_PICHU_DOLL,
        DecorGfx_PICHU_DOLL
    }, {
        DECOR_PIKACHU_DOLL,
        TEXT_DECORNAME_PIKACHU_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_DOLL,
        3000,
        DecorDesc_PIKACHU_DOLL,
        DecorGfx_PIKACHU_DOLL
    }, {
        DECOR_MARILL_DOLL,
        TEXT_DECORNAME_MARILL_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_DOLL,
        3000,
        DecorDesc_MARILL_DOLL,
        DecorGfx_MARILL_DOLL
    }, {
        DECOR_TOGEPI_DOLL,
        TEXT_DECORNAME_TOGEPI_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_DOLL,
        3000,
        DecorDesc_TOGEPI_DOLL,
        DecorGfx_TOGEPI_DOLL
    }, {
        DECOR_CYNDAQUIL_DOLL,
        TEXT_DECORNAME_CYNDAQUIL_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_DOLL,
        3000,
        DecorDesc_CYNDAQUIL_DOLL,
        DecorGfx_CYNDAQUIL_DOLL
    }, {
        DECOR_CHIKORITA_DOLL,
        TEXT_DECORNAME_CHIKORITA_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_DOLL,
        3000,
        DecorDesc_CHIKORITA_DOLL,
        DecorGfx_CHIKORITA_DOLL
    }, {
        DECOR_TOTODILE_DOLL,
        TEXT_DECORNAME_TOTODILE_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_DOLL,
        3000,
        DecorDesc_TOTODILE_DOLL,
        DecorGfx_TOTODILE_DOLL
    }, {
        DECOR_JIGGLYPUFF_DOLL,
        TEXT_DECORNAME_JIGGLYPUFF_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_DOLL,
        3000,
        DecorDesc_JIGGLYPUFF_DOLL,
        DecorGfx_JIGGLYPUFF_DOLL
    }, {
        DECOR_MEOWTH_DOLL,
        TEXT_DECORNAME_MEOWTH_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_DOLL,
        3000,
        DecorDesc_MEOWTH_DOLL,
        DecorGfx_MEOWTH_DOLL
    }, {
        DECOR_CLEFAIRY_DOLL,
        TEXT_DECORNAME_CLEFAIRY_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_DOLL,
        3000,
        DecorDesc_CLEFAIRY_DOLL,
        DecorGfx_CLEFAIRY_DOLL
    }, {
        DECOR_DITTO_DOLL,
        TEXT_DECORNAME_DITTO_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_DOLL,
        3000,
        DecorDesc_DITTO_DOLL,
        DecorGfx_DITTO_DOLL
    }, {
        DECOR_SMOOCHUM_DOLL,
        TEXT_DECORNAME_SMOOCHUM_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_DOLL,
        3000,
        DecorDesc_SMOOCHUM_DOLL,
        DecorGfx_SMOOCHUM_DOLL
    }, {
        DECOR_TREECKO_DOLL,
        TEXT_DECORNAME_TREECKO_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_DOLL,
        3000,
        DecorDesc_TREECKO_DOLL,
        DecorGfx_TREECKO_DOLL
    }, {
        DECOR_TORCHIC_DOLL,
        TEXT_DECORNAME_TORCHIC_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_DOLL,
        3000,
        DecorDesc_TORCHIC_DOLL,
        DecorGfx_TORCHIC_DOLL
    }, {
        DECOR_MUDKIP_DOLL,
        TEXT_DECORNAME_MUDKIP_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_DOLL,
        3000,
        DecorDesc_MUDKIP_DOLL,
        DecorGfx_MUDKIP_DOLL
    }, {
        DECOR_DUSKULL_DOLL,
        TEXT_DECORNAME_DUSKULL_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_DOLL,
        3000,
        DecorDesc_DUSKULL_DOLL,
        DecorGfx_DUSKULL_DOLL
    }, {
        DECOR_WYNAUT_DOLL,
        TEXT_DECORNAME_WYNAUT_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_DOLL,
        3000,
        DecorDesc_WYNAUT_DOLL,
        DecorGfx_WYNAUT_DOLL
    }, {
        DECOR_BALTOY_DOLL,
        TEXT_DECORNAME_BALTOY_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_DOLL,
        3000,
        DecorDesc_BALTOY_DOLL,
        DecorGfx_BALTOY_DOLL
    }, {
        DECOR_KECLEON_DOLL,
        TEXT_DECORNAME_KECLEON_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_DOLL,
        3000,
        DecorDesc_KECLEON_DOLL,
        DecorGfx_KECLEON_DOLL
    }, {
        DECOR_AZURILL_DOLL,
        TEXT_DECORNAME_AZURILL_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_DOLL,
        3000,
        DecorDesc_AZURILL_DOLL,
        DecorGfx_AZURILL_DOLL
    }, {
        DECOR_SKITTY_DOLL,
        TEXT_DECORNAME_SKITTY_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_DOLL,
        3000,
        DecorDesc_SKITTY_DOLL,
        DecorGfx_SKITTY_DOLL
    }, {
        DECOR_SWABLU_DOLL,
        TEXT_DECORNAME_SWABLU_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_DOLL,
        3000,
        DecorDesc_SWABLU_DOLL,
        DecorGfx_SWABLU_DOLL
    }, {
        DECOR_GULPIN_DOLL,
        TEXT_DECORNAME_GULPIN_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_DOLL,
        3000,
        DecorDesc_GULPIN_DOLL,
        DecorGfx_GULPIN_DOLL
    }, {
        DECOR_LOTAD_DOLL,
        TEXT_DECORNAME_LOTAD_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_DOLL,
        3000,
        DecorDesc_LOTAD_DOLL,
        DecorGfx_LOTAD_DOLL
    }, {
        DECOR_SEEDOT_DOLL,
        TEXT_DECORNAME_SEEDOT_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_DOLL,
        3000,
        DecorDesc_SEEDOT_DOLL,
        DecorGfx_SEEDOT_DOLL
    }, {
        DECOR_PIKA_CUSHION,
        TEXT_DECORNAME_PIKA_CUSHION,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_CUSHION,
        2000,
        DecorDesc_PIKA_CUSHION,
        DecorGfx_PIKA_CUSHION
    }, {
        DECOR_ROUND_CUSHION,
        TEXT_DECORNAME_ROUND_CUSHION,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_CUSHION,
        2000,
        DecorDesc_ROUND_CUSHION,
        DecorGfx_ROUND_CUSHION
    }, {
        DECOR_KISS_CUSHION,
        TEXT_DECORNAME_KISS_CUSHION,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_CUSHION,
        2000,
        DecorDesc_KISS_CUSHION,
        DecorGfx_KISS_CUSHION
    }, {
        DECOR_ZIGZAG_CUSHION,
        TEXT_DECORNAME_ZIGZAG_CUSHION,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_CUSHION,
        2000,
        DecorDesc_ZIGZAG_CUSHION,
        DecorGfx_ZIGZAG_CUSHION
    }, {
        DECOR_SPIN_CUSHION,
        TEXT_DECORNAME_SPIN_CUSHION,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_CUSHION,
        2000,
        DecorDesc_SPIN_CUSHION,
        DecorGfx_SPIN_CUSHION
    }, {
        DECOR_DIAMOND_CUSHION,
        TEXT_DECORNAME_DIAMOND_CUSHION,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_CUSHION,
        2000,
        DecorDesc_DIAMOND_CUSHION,
        DecorGfx_DIAMOND_CUSHION
    }, {
        DECOR_BALL_CUSHION,
        TEXT_DECORNAME_BALL_CUSHION,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_CUSHION,
        2000,
        DecorDesc_BALL_CUSHION,
        DecorGfx_BALL_CUSHION
    }, {
        DECOR_GRASS_CUSHION,
        TEXT_DECORNAME_GRASS_CUSHION,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_CUSHION,
        2000,
        DecorDesc_GRASS_CUSHION,
        DecorGfx_GRASS_CUSHION
    }, {
        DECOR_FIRE_CUSHION,
        TEXT_DECORNAME_FIRE_CUSHION,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_CUSHION,
        2000,
        DecorDesc_FIRE_CUSHION,
        DecorGfx_FIRE_CUSHION
    }, {
        DECOR_WATER_CUSHION,
        TEXT_DECORNAME_WATER_CUSHION,
        DECORPERM_SPRITE,
        DECORSHAPE_1x1,
        DECORCAT_CUSHION,
        2000,
        DecorDesc_WATER_CUSHION,
        DecorGfx_WATER_CUSHION
    }, {
        DECOR_SNORLAX_DOLL,
        TEXT_DECORNAME_SNORLAX_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x2,
        DECORCAT_DOLL,
        10000,
        DecorDesc_SNORLAX_DOLL,
        DecorGfx_SNORLAX_DOLL
    }, {
        DECOR_RHYDON_DOLL,
        TEXT_DECORNAME_RHYDON_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x2,
        DECORCAT_DOLL,
        10000,
        DecorDesc_RHYDON_DOLL,
        DecorGfx_RHYDON_DOLL
    }, {
        DECOR_LAPRAS_DOLL,
        TEXT_DECORNAME_LAPRAS_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x2,
        DECORCAT_DOLL,
        10000,
        DecorDesc_LAPRAS_DOLL,
        DecorGfx_LAPRAS_DOLL
    }, {
        DECOR_VENUSAUR_DOLL,
        TEXT_DECORNAME_VENUSAUR_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x2,
        DECORCAT_DOLL,
        10000,
        DecorDesc_VENUSAUR_DOLL,
        DecorGfx_VENUSAUR_DOLL
    }, {
        DECOR_CHARIZARD_DOLL,
        TEXT_DECORNAME_CHARIZAR_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x2,
        DECORCAT_DOLL,
        10000,
        DecorDesc_CHARIZARD_DOLL,
        DecorGfx_CHARIZARD_DOLL
    }, {
        DECOR_BLASTOISE_DOLL,
        TEXT_DECORNAME_BLASTOISE_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x2,
        DECORCAT_DOLL,
        10000,
        DecorDesc_BLASTOISE_DOLL,
        DecorGfx_BLASTOISE_DOLL
    }, {
        DECOR_WAILMER_DOLL,
        TEXT_DECORNAME_WAILMER_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x2,
        DECORCAT_DOLL,
        10000,
        DecorDesc_WAILMER_DOLL,
        DecorGfx_WAILMER_DOLL
    }, {
        DECOR_REGIROCK_DOLL,
        TEXT_DECORNAME_REGIROCK_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x2,
        DECORCAT_DOLL,
        10000,
        DecorDesc_REGIROCK_DOLL,
        DecorGfx_REGIROCK_DOLL
    }, {
        DECOR_REGICE_DOLL,
        TEXT_DECORNAME_REGICE_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x2,
        DECORCAT_DOLL,
        10000,
        DecorDesc_REGICE_DOLL,
        DecorGfx_REGICE_DOLL
    }, {
        DECOR_REGISTEEL_DOLL,
        TEXT_DECORNAME_REGISTEEL_DOLL,
        DECORPERM_SPRITE,
        DECORSHAPE_1x2,
        DECORCAT_DOLL,
        10000,
        DecorDesc_REGISTEEL_DOLL,
        DecorGfx_REGISTEEL_DOLL}
};
